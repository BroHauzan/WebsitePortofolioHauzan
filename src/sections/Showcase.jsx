...
import { showcaseIcons } from '../components/Icons';
...
export default function Showcase() {
  ...
  return (
    <section
      ...
      className="border-t border-cream-border py-20 sm:py-28 max-w-7xl mx-auto px-6 sm:px-10 scroll-mt-20 overflow-hidden"
      id="works"
      role="region"
    >
        ...
        >
          {showcaseCategories.map((category, i) => (
            <div
              ...
              className="coverflow-card-item w-[320px] sm:w-[360px] md:w-[380px] -ml-[160px] sm:-ml-[180px] md:-ml-[190px] -mt-[245px] sm:-mt-[255px] bg-white border border-cream-border rounded-2xl overflow-hidden cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900/20 focus-visible:ring-offset-2"
            ...
            >
              <div className="aspect-[16/10] bg-[#f4f4f0] border-b border-cream-border relative overflow-hidden flex items-center justify-center p-6 text-center">
                <div className="flex flex-col items-center">
                  <div className="w-12 h-12 rounded-full border border-slate-300 bg-white/80 flex items-center justify-center text-slate-600 mb-3 shadow-sm">
                    {showcaseIcons[category.key]}
                  </div>
                  <span className="font-display font-medium text-slate-900 text-sm tracking-tight">{category.subtitle}</span>
                  <span className="text-[10px] font-mono uppercase tracking-[0.14em] text-slate-500 mt-1">{category.visualMeta}</span>
                </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] font-mono uppercase tracking-wider text-slate-400">{category.collection}</span>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-cream-border text-xs font-medium text-ink-primary">
                  <span className="text-[11px] font-mono text-slate-500">{category.tools}</span>
                  <a
                    className="card-action-link inline-flex items-center gap-1 group-hover:translate-x-1 transition-transform text-slate-900 font-semibold hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900/20 focus-visible:ring-offset-2 rounded"
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-14 px-2">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="w-11 h-11 rounded-full border border-cream-border bg-white text-ink-primary hover:bg-slate-900 hover:text-white transition-all flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900/20 focus-visible:ring-offset-2 shadow-sm cursor-pointer"
            type="button"
          ...
          {showcaseCategories.map((category, i) => {
            ...
            return (
              <button
                ...
                aria-label={`Pilih Kategori ${i + 1}: ${category.title}`}
                aria-current={isActive ? 'true' : undefined}
                className={`indicator-dot rounded-full transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900/20 focus-visible:ring-offset-2 ${
      ...
      <div className="bg-white border border-cream-border rounded-xl p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pb-4 border-b border-cream-border gap-2">
          <div>
            <h3 className="text-[11px] font-semibold tracking-[0.14em] uppercase text-ink-muted">KATALOG TERBUKA: VIDEOGRAFI PILIHAN</h3>
            <p className="text-xs text-ink-muted mt-0.5">Dokumentasi utama yang diproduksi bersama tim Media Center &amp; OSIS IT Pubdok.</p>
          </div>
          <a className="text-xs font-semibold text-slate-900 hover:text-slate-600 transition-colors inline-flex items-center gap-1.5 self-start sm:self-auto focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900/20 focus-visible:ring-offset-2 rounded" href="#works">
            <span className="">Jelajahi Galeri Penuh</span>
            <span aria-hidden="true" className="">↗</span>
          </a>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-xs">
          <div className="border-l border-cream-border pl-4">
            <span className="text-[10px] font-mono uppercase tracking-wider text-slate-400 block mb-1">Live Concert &amp; Stage</span>
          ...
          <div className="border-l border-cream-border pl-4">
            <span className="text-[10px] font-mono uppercase tracking-wider text-slate-400 block mb-1">Recruitment &amp; Showcase</span>
          ...
          <div className="border-l border-cream-border pl-4">
            <span className="text-[10px] font-mono uppercase tracking-wider text-slate-400 block mb-1">Ceremonial Broadcast</span>

...
export default function Footer() {
  return (
    <footer className="w-full bg-[#090e1a] text-[#f8fafc] border-t border-slate-800 mt-20 scroll-mt-20" id="contact">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-16">
          <div className="md:col-span-5 flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.14em] text-slate-400 font-medium mb-4">MULTIMEDIA PRODUCTION &amp; VISUAL ARTS</p>
              <p className="text-sm text-slate-400 leading-relaxed max-w-sm">
                Kurasi dokumentasi visual, penyusunan narasi kreatif, dan eksplorasi multimedia SMAN 1 Lumajang.
              </p>
            </div>
            <div className="mt-8 pt-8 border-t border-slate-800/80 text-xs text-slate-500 flex flex-wrap items-center justify-between gap-4">
              <span className="">© 2026 Hauzan Naufal. All rights reserved.</span>
          ...
          <div className="md:col-span-7 grid grid-cols-2 gap-8 text-sm">
            <div>
              <h3 className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-400 mb-4">Navigation</h3>
              <ul className="space-y-2.5 text-slate-300">
                {footerNavLinks.map((link) => (
                  <li key={link.href} className="">
                    <a className="hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 rounded" href={link.href}>
                      {link.label}
                    </a>
                  </li>
                ))}
            ...
            <div>
              <h3 className="text-xs font-semibold tracking-[0.14em] uppercase text-slate-400 mb-4">Contact</h3>
              ...
              <ul className="space-y-2 text-xs text-slate-400">
                {socialLinks.map((link) => (
                  <li key={link.href} className="">
                    <a className="hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 rounded" href={link.href} rel="noopener noreferrer" target="_blank">
                      {link.label}
                    </a>
                  </li>
                ))}
